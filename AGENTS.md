...
- **Bump the app version only when the owner says a build is being made.** No OTA updates
  exist, so changes ship only via a new store build and Play rejects a reused `versionCode`.
  Bump **only** when the owner says they ran / are running a build — never on merge or
